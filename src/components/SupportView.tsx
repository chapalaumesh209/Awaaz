      ...
      <div className="bg-white border border-teal-100 rounded-3xl p-6 sm:p-8 shadow-sm">
        ...
        {submitted ? (
          <div className="text-center py-8 space-y-4">
            ...
            <h3 className="font-sans text-xl font-bold text-gray-900">{t('Request Dispatched Successfully!')}</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center space-x-2.5 border-b border-gray-100 pb-4 mb-4">
              ...
              <h3 className="font-sans text-base font-bold text-gray-800">{t('Support Enrollment Form')}</h3>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">{t('What do you need help with?')}</label>
                <select
                  value={category}
                ...
                >
                  <option value="scheme_help">{t('Scheme Eligibility & Application Filing')}</option>
                  <option value="document_help">{t('Identity Papers Correction (Aadhaar/Ration Card)')}</option>
                  <option value="caste_income_help">{t('Income/Caste Certificate filing')}</option>
                  <option value="disability_udid">{t('Disability UDID registration')}</option>
                  <option value="other_support">{t('Other Direct Administrative Help')}</option>
                </select>
              </div>

              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">{t('Priority Level')}</label>
                <select
                  value={priority}
                ...
                >
                  ...
                  <option value="medium">{t('Medium Priority (Need help in 3-4 days)')}</option>
                  <option value="high">{t('High Priority (Urgent requirement, close deadline)')}</option>
                </select>
              </div>
            </div>

            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">{t('Explain your situation in simple words')}</label>
              <textarea
                required
                rows={4}
                value={notes}
                onChange={(e) => setNotes(e.target.value)}
                placeholder={t("Example: I want to apply for PM Vishwakarma, but I do not have a registered income certificate. I need a volunteer to help file the Tahsildar forms.")}
                className="w-full bg-gray-50/50 border border-gray-200 rounded-xl px-3.5 py-2.5 text-xs font-medium focus:ring-1 focus:ring-teal-500 focus:outline-hidden"
              />
            </div>
            ...
            <div className="bg-teal-50/30 p-4 rounded-2xl border border-teal-100/50">
              <span className="block text-xs font-bold text-teal-950">{t('Safety Statement')}</span>
            ...
            >
              ...
              <span>{t('Dispatch Request to Volunteer Queue')}</span>
            </button>
